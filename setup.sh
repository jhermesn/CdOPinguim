...
# -----------------------------------------------------------------------------
# Oficina Linux - Setup "Cd o Pinguim" (WSL/Ubuntu)
# Cria uma estrutura aleatória com pistas e um secret.txt para os participantes.
# Ao final, remove automaticamente a pasta do repositório clonado.
# -----------------------------------------------------------------------------
...
random_hex() {
  ...
  if command -v openssl >/dev/null 2>&1; then
    ...
    hexdump -vn "$bytes" -e '16/1 "%02x"' /dev/urandom
  fi
}
...
# HMAC-SHA256 helper (hex output). Uses openssl if available, falls back to python3
hmac_sha256_hex() {
...
d=hmac.new(key, data, hashlib.sha256).hexdigest()
print(d)
PY
  fi
}
...
derive_flag_from_fingerprint() {
  # derive_flag_from_fingerprint <salt> <challenge>
  local salt="$1"; local challenge="$2"
  local fp
  fp="$(compute_fingerprint_string)"
  local data="challenge=${challenge}|fingerprint=${fp}"
  local mac
  mac="$(hmac_sha256_hex "$salt" "$data")" || die "Falha ao gerar HMAC"
  # Truncar para 24 hex (12 bytes) para tamanho amigável
  local short="${mac:0:24}"
  printf "FLAG{%s}\n" "$short"
}

embed_flag_in_template() {
  # embed_flag_in_template <template_path> <output_path> <flag>
  local template="$1"; local out="$2"; local flag="$3"
  if [ -r "$template" ]; then
    awk -v repl="$flag" '{gsub(/\{\{SECRET\}\}/, repl); print $0}' "$template" >"$out"
  else
    printf "%s\n" "$flag" >"$out"
  fi
}
...
make_decoys_in_dir() {
  # make_decoys_in_dir <dir> <qtd_files>
  local target="$1" ; local count="${2:-8}"
  mkdir -p "$target"
  local -a exts=("txt" "log" "conf" "md" "dat" "" "bak")
  for _ in $(seq 1 "$count"); do
    local name="$(random_letters 6)"
    local ext="${exts[$((RANDOM % ${#exts[@]}))]}"
    local file="$target/$name"
    [[ -n "$ext" ]] && file="$file.$ext"
    {
      for i in $(seq 1 $((3 + RANDOM % 9))); do
        printf "linha %02d: %s\n" "$i" "$(random_hex 8)"
      done
    } >"$file"
  done
...
create_tar_with_file() {
  # create_tar_with_file <tar_path> <filename_inside> <content>
  local tar_path="$1" ; local inner="$2" ; local content="$3"
  local tmpd=""
  tmpd="$(mktemp -d)"
  printf "%s\n" "$content" >"$tmpd/$inner"
  tar -C "$tmpd" -czf "$tar_path" "$inner"
  rm -rf "$tmpd"
}
...
main() {
  ...
  if ! is_wsl; then
    log "Aviso: não parece ser WSL; seguindo assim mesmo."
  fi

  # Diretório base aleatório sob $HOME
  local base_name=".$(random_letters 6)-$(random_hex 4)"
  local base_dir="${HOME}/${base_name}"
  mkdir -p "$base_dir"

  # Parâmetros de desafio (para verificação determinística)
  ...
  if [[ -n "${CTF_SALT:-}" ]]; then
    ...
    salt="$(random_hex 16)"
    log "CTF_SALT não informado; gerado automaticamente. Salve para verificação: $salt"
  fi

  # Derivar FLAG determinística por máquina/desafio
  local flag
  flag="$(derive_flag_from_fingerprint "$salt" "$challenge")"

  # README com instruções ao participante
...
EOF
  ...
  # Gerar uma coleção de diretórios e subdiretórios aleatórios
  local -a dirs=()
  for i in $(seq 1 8); do
  ...
  # Adicionar um diretório oculto com um .tar.gz contendo a 1ª pista
  ...
  create_tar_with_file \
    "${hidden_dir}/${tar_name}" \
    "pista1.txt" \
    "Pista 1: Existe um arquivo de texto em algum lugar contendo a palavra PINGUIM.
Dica: use grep recursivo para encontrá-lo (ex.: grep -R \"PINGUIM\" .)."

  # Decoys em todos os diretórios
  for d in "${dirs[@]}"; do
    make_decoys_in_dir "$d" $((6 + RANDOM % 10))
  done
  make_decoys_in_dir "$hidden_dir" 5

  # Escolher diretórios aleatórios para colocar pistas seguintes
  ...
  # Diretório destino do symlink (com permissão restrita)
  local secret_container="${base_dir}/$(random_letters 6)/$(random_letters 6)"
  mkdir -p "$secret_container"

  # Dentro do secret_container: um .tar.gz com uma pista em base64
...
Se não estiver no diretório base, navegue até onde começou a dinâmica."
  ...
  # Restringir leitura no secret_container (x-only), forçando pensar em permissões
  chmod 111 "$secret_container"

  # Colocar arquivo com PINGUIM e dica do symlink
...
EOF
  ...
  # Adicionar um script executável como distração/dica
...
EOF
  ...
  # Criar o link simbólico "atalho" apontando para o secret_container
  ln -s "$secret_container" "${dir_with_symlink}/atalho"

  # Criar caminho profundo e oculto para o secret.txt
  ...
  # Também salvar referência reduzida no HOME (para organizador)
  {
    printf '{"salt":"%s",' "$salt"
    printf '"challenge":"%s"}' "$challenge"
  } > "$HOME/.ctf_pinguim.json"

  # Um arquivo-fantasma para confundir (parece texto mas é gzip)
  ...
  # Saída final (curta, sem revelar o caminho do secret)
